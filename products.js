...
function draw(cat='همه'){const list=cat==='همه'?PRODUCTS:PRODUCTS.filter(p=>p.cat===cat);root.innerHTML=list.map(p=>`<a class="catalog-card" href="${productUrl(p)}"><div class="catalog-image"><img src="${p.image}" alt="${p.name}"><span>${p.cat}</span></div><div><small>${p.en}</small><h2>${p.name}</h2><p>${p.summary.slice(0,125)}…</p><b>مشاهده مشخصات کامل ←</b></div></a>`).join('')}
